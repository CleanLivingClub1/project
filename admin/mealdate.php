<?php
	...
	if(isset($_SESSION['admin']))
	{
		...
		if(isset($_REQUEST['savenewmealiteams']))
		{
			...
			echo move_uploaded_file($_FILES['picture']['tmp_name'],USER_UPLOAD_PATH.$pictureName);
		...
		{	
			...
			$pictureName  					= $_FILES['picture']['name'];
			if (move_uploaded_file($_FILES['picture']['tmp_name'],USER_UPLOAD_PATH.$pictureName)) 
			{		$valArray['picture'] = $pictureName;echo "file sent";}else 	{	echo "file not sent";}
			$videoName  					= $_FILES['video']['name'];
			if (move_uploaded_file($_FILES['video']['tmp_name'],USER_UPLOAD_PATH.$videoName)) 
			{	$valArray['video'] = $videoName;	echo "file sent";	}	else {	echo "file not sent";	}
	...
	$smarty->display('file:[admin]mealdate.tpl');
	}
	else
