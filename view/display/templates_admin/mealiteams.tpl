                                ...
                                <div class="row">
                                   ...
                                   {foreach $mealiteams as $key=>$val}
									...
									{/if}
                                            ...
                                            <div class="col-md-2">{$val.mealtypename}</div>
                                            <div class="col-md-1">{$val.title}</div>
                                            <div class="col-md-2">{$val.description}</div>
                                            <div class="col-md-1"><img src="{$TEMPLATE}upload/{$val.picture}" class="image"/></div>
                                            <div class="col-md-1"><img src="{$TEMPLATE}upload/{$val.video}" class="image"/></div>
									...
									<div id="deletevalue{$val.id}"></div>
										...
										$(document).ready(function()
										{
											$("#edit{$val.id}").click(function(){
												...
												var mid	= $("#editVal{$val.id}").val();
												
												$("#editvalue{$val.id}").load("mealiteams.php?editmealitem="+mid);
											});
											...
											var check	=	confirm("Are you sure to delete?");
												...
												{							
													...
													$("#deletevalue{$val.id}").load("mealiteams.php?deletemealitem="+mid);
												}
											});
										});
										</script>
